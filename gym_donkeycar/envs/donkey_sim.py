...
class DonkeyUnitySimHandler(IMesgHandler):
    ...
    def is_game_over(self) -> bool:
        ...
        if abs(self.cte) > self.max_cte or self.speed  > 1:
            self.over = True
    ...
    def calc_reward(self, done: bool) -> float:
        ...
        if self.forward_vel > 0.0:
            reward_t = 10*(1.0 - (math.fabs(self.cte) / self.max_cte)) * self.forward_vel
            reward = reward_t - self.reward
            self.reward = reward_t

            return reward

        reward_t = 100*self.forward_vel
        self.reward = reward_t

        return reward_t # in reverse, reward doesn't have centering term as this can result in some exploits
